    ...
    <div>
        <div class="metoo-filter-container">
            ...
            <v-flex xs12 md6 lg4 class="pl-3 pr-3 pt-4">
                <div class="metoo-container-main-title-history">История</div>
            </v-flex>
...
<script>
    ...
    export default {
        ...
        computed: {
            events: function() {
                let events = this.$root.$store.state.events.history;
        ...
        methods: {
            ...
            loadHistory: function () {
                if (this.$root.$route.name == "history") {
                    /*this.$root.$store.dispatch('loadHistory');*/
                    let csrf = this.$root.$store.state.system.csrf.content;
                    api.events.getHistory({headers: {'X-XSRF-TOKEN': csrf }} ).then(
                        response => { // success callback
                            ...
                            this.$root.$store.commit('EVENTS_HISTORY_SET', events);
                        },
                        response => { // error callback
        ...
        watch: {
            '$route': 'loadHistory' // в случае изменения маршрута запрашиваем данные вновь (перешли назад с другой стр))
        },
        created () {
            this.loadHistory();
        },
...
<style>
    .metoo-container-main-title-history {
